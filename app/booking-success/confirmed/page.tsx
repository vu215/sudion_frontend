      ...
      <main className="min-h-screen bg-[#fafbfc] text-[#0e111d]">
        <section className="mx-auto grid w-full max-w-[860px] gap-5 px-6 py-20 md:px-12 lg:px-20">
          ...
          <h1 className="max-w-[680px] text-[38px] font-black leading-[1.08] tracking-[-0.03em] text-[#0e111d] sm:text-[48px]">
            Mã booking không tồn tại hoặc đã bị xóa trên trình duyệt này
          </h1>
      ...
      <section className="mx-auto w-full max-w-[760px] px-6 py-20">
        <div className="mx-auto w-full rounded-[20px] bg-white p-8 shadow-[0_20px_40px_rgba(15,23,42,0.08)]">
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="text-[26px] font-black">Đặt lịch thành công!</h1>
            <p className="mt-3 max-w-[620px] text-[15px] text-[#475569]">
              Chúc mừng! Lịch hẹn của bạn đã được xác nhận. Chúng tôi đã gửi email thông tin chi tiết.
            </p>

            <div className="mt-8 w-full rounded-[12px] border border-[#eef0f5] bg-white p-6 shadow-sm relative">
              <div className="absolute right-6 top-6 text-[#ff8d28] font-black">{booking.id}</div>

              <div className="flex items-center gap-4">
                <img
                  src={booking.referenceFileName ? `/uploads/${booking.referenceFileName}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(booking.photographerName)}&background=ffffff&color=333&size=128`}
                  alt={booking.photographerName}
                  className="h-20 w-20 rounded-lg object-cover"
                />
                <div className="min-w-0">
                  <p className="text-[16px] font-black">{booking.photographerName}</p>
                  <p className="mt-1 text-[13px] text-[#ff8d28] font-extrabold">★ 4.9 (120 đánh giá)</p>
                </div>
              </div>

              <div className="mt-6 grid gap-3 rounded-[8px] bg-[#fffaf5] p-4">
                <div className="flex items-center gap-3">
                  <div className="rounded-full bg-[#fff3e6] p-2 text-[#ff8d28]">📅</div>
                  <div>
                    <div className="text-[13px] font-semibold">Ngày hẹn</div>
                    <div className="text-[13px] text-[#475569]">{formatDate(booking.shootDate)}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[13px] text-[#475569]">{formatTimeRange(booking.shootTime)}</div>
                  </div>
                </div>

                <div className="flex items-center gap-3">
                  <div className="rounded-full bg-[#fff3e6] p-2 text-[#ff8d28]">🎞️</div>
                  <div>
                    <div className="text-[13px] font-semibold">Gói dịch vụ</div>
                    <div className="text-[13px] text-[#475569]">{booking.serviceName}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 border-t border-[#f1f3f7] pt-4 text-center">
                <Link href="/" className="text-[13px] text-[#2563eb]">← Quay lại Trang chủ</Link>
              </div>
            </div>
          </div>
        </div>
