...
export default function RefundsPage() {
  ...
  async function loadRefunds() {
    ...
    try {
      ...
      console.error("Lỗi lấy dữ liệu refund từ API:", error);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  async function loadStats() {
    try {
      ...
      console.error("Lỗi lấy stats refund:", error);
    }
  }

  useEffect(() => {
    loadRefunds();
  }, [page, query, status, method]);
  ...
  const handleRejectRefund = async (refundId: string) => {
    try {
      const result = await api.refunds.reject(refundId, adminNote);
      if (result.success) {
        patch(refundId, { status: "Từ chối", adminNote });
      ...
      notify("Lỗi khi từ chối yêu cầu hoàn tiền.");
    }
  };

  const handleSaveNote = async (refundId: string) => {
    try {
      const result = await api.refunds.update(refundId, { admin_note: adminNote });
      if (result.success) {
        patch(refundId, { adminNote });
  ...
  const kpiPendingCount = stats?.pending ?? items.filter((i) => i.status === "Chờ duyệt").length;
  const kpiPendingAmount = stats ? Number(stats.pending_amount) : totalPending;
  const kpiProcessedCount = stats?.processed ?? items.filter((i) => i.status === "Đã hoàn tiền").length;
  const kpiProcessedAmount = stats ? Number(stats.processed_amount) : totalRefunded;
  const kpiRejectedCount = stats?.rejected ?? items.filter((i) => i.status === "Từ chối").length;
    ...
    <AdminLayout active="Hoàn tiền" search={query} onSearch={setQuery}>
      ...
      <div className="min-w-0">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard title="Tổng yêu cầu" value={String(kpiTotal)} note="Trong hệ thống" tone="orange" />
          <StatCard title="Chờ duyệt" value={String(kpiPendingCount)} note={`${money(kpiPendingAmount)} đang chờ`} tone="amber" />
          <StatCard title="Đã hoàn tiền" value={String(kpiProcessedCount)} note={money(kpiProcessedAmount)} tone="green" />
          <StatCard title="Từ chối" value={String(kpiRejectedCount)} note="Không đủ điều kiện" tone="red" />
        </div>
        ...
        <Panel className="mt-4">
          <div className="mb-3 flex gap-6 overflow-x-auto border-b border-[#edf0f5]">
            {(["Tất cả", "Chờ duyệt", "Đang xử lý", "Đã hoàn tiền", "Từ chối"] as const).map((tab) => (
              <button key={tab} onClick={() => setStatus(tab)} className={`shrink-0 border-b-2 px-2 py-3 text-[12px] font-medium ${status === tab ? "border-[#ff8d28] text-[#ff8d28]" : "border-transparent text-[#536078]"}`}>
                {tab} ({tab === "Tất cả" ? kpiTotal : tab === "Chờ duyệt" ? kpiPendingCount : tab === "Đang xử lý" ? (stats?.approved ?? items.filter((i) => i.status === "Đang xử lý").length) : tab === "Đã hoàn tiền" ? kpiProcessedCount : kpiRejectedCount})
              </button>
            ))}
          </div>
            ...
            <div className="overflow-x-auto rounded-xl border border-[#e6e9f1]">
              <table className="w-full min-w-[1040px] text-left text-[12px]">
                ...
                <tbody className="divide-y divide-[#edf0f5]">
                  {filtered.length === 0 ? (
                    <tr><td colSpan={10} className="px-3 py-8 text-center text-[#697086]">Không tìm thấy yêu cầu hoàn tiền nào.</td></tr>
                  ) : (
                    filtered.map((item) => (
                      <tr key={item.id} onClick={() => openDetail(item.id)} className={`cursor-pointer hover:bg-[#fff8f1] ${selectedId === item.id ? "bg-[#fff3e8]" : "bg-white"}`}>
                        ...
                        <td className="px-3 py-3"><div className="flex gap-2">
                          ...
                          {item.status === "Chờ duyệt" && <IconButton label="Duyệt" icon="check" onClick={(e) => { e.stopPropagation(); handleBankRefund(item.id, "Duyệt"); }} />}
                        </div></td>
                      </tr>
                    ))
                  )}
          ...
          <div className="mt-4 flex items-center justify-between text-[12px] text-[#697086]">
            <span>Hiển thị 1 - {filtered.length} của {pagination?.total || filtered.length} yêu cầu</span>
            <div className="flex gap-2">
          ...
          <aside className={`absolute right-0 top-0 h-full w-full min-w-0 overflow-y-auto border-l border-[#e6e9f1] bg-white px-5 py-5 shadow-[-24px_0_48px_rgba(12,18,32,0.2)] transition-transform duration-300 ease-out sm:w-[430px] ${detailOpen ? "translate-x-0" : "translate-x-full"}`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-5 flex items-center gap-3 border-b border-[#edf0f5] pb-5">
              ...
              <div>
                ...
                <p className="text-[12px] text-[#697086]">{selected.customerId}</p>
                <p className="mt-1 text-[12px] text-[#697086]">{selected.email} · {selected.phone}</p>
              </div>
            </div>
